...
private PredictionType classType;
 ...
 * @param map 
   */
  public TreeBuild(Map<String, Color> map, PredictionType classType) {
 ...
 * @param maxValue 
   ...
   */
  public Node create(Reader t, Map<String, PredictionType> variableType, Character charDelimiter, float maxValue) {
    ...
    Node top = generateStructures(variableType,charDelimiter,maxValue);
 ...
 * @param charDelimiter 
   ...
   */
  private Node generateStructures(Map<String, PredictionType> variableType, Character charDelimiter,float maxVal) {
    String id, label; // ,source,target; NOT USED
    Integer shape, style;
    // int fontsize; NOT USED
    Color fontcolor, color=null;
    ...
    for (int noa = 0; noa < m_nodes.size(); noa++) {
      ...
      if (color==null && maxVal>0)
      {
    	  String[] split = label.split(" ");
    	  try
    	  {
    		  float value=(float) (0.15+0.85*Math.abs(Float.parseFloat(split[0])/maxVal));
    		  if (value<0)
    			  value=0;
    		  if (value<0.5)
    			  color=new Color(1F,1F-2F*value,1F-2F*value);
    		  else
    			  color=new Color(2F*(1-value),0,0);
    	  }
		  ...
		  try
		  {
			  BigDecimal bg = new BigDecimal(split[0]);
			  if (classType==PredictionType.TIMESTAMP)
				  split[0]=new Date(bg.longValue()).toString();
			  else if (classType==PredictionType.TIMEINTERVAL)
			  {
				  float value=bg.floatValue();
				  if (value<1000)
					  split[0]=bg.longValue()+ " millisecs";
				  else if (value < 60000)
					  split[0]=value/1000F+ " secs";
				  else if (value < 3600000)
					  split[0]=value/60000F+ " mins";
				  else if (value < 86400000F)
					  split[0]=value/3600000F+ " hours";
				  else
					  split[0]=value/86400000F+ " days";
			  }
			  label="";
      ...
      {
    	  double y = 0.2126 * Math.pow(color.getRed()/255F,2.2)  +  
    			  0.7151 * Math.pow(color.getGreen()/255F,2.2)  +  
    			  0.0721 * Math.pow(color.getBlue()/255F,2.2);
    	  if (y<0.18)
      ...
      m_aNodes.addElement(new Node(label, id, style.intValue(), shape
        .intValue(), color, t.m_data,fontcolor));
    }
    ...
    for (int noa = 0; noa < m_edges.size(); noa++) {
      ...
      String nodeLabel=getNodeByReference(t.m_source);
      if (charDelimiter!=null)
      {
    	  int index=nodeLabel.indexOf(charDelimiter);
    	  if (index>0)
    		  nodeLabel=nodeLabel.substring(index+1).trim();
      }
      PredictionType type=variableType.get(nodeLabel);
      String[] split=label.split(" ");
      if (type!=null)
      {
    	  switch(type)
    	  {
    		  case BOOLEAN :
    			  if (split[split.length-1].startsWith("F") || split[split.length-1].startsWith("0") || split[split.length-1].startsWith("f"))
    				  split[split.length-1]="False";
    			  else
    		  ...
    		  case TIMEINTERVAL:
    			  try
    			  {
    				  BigDecimal bg = new BigDecimal(split[split.length-1]);
    				  if (type==PredictionType.TIMESTAMP)
    					  split[split.length-1]=new Date(bg.longValue()).toString();
    				  else
    				  {
    					  float value=bg.floatValue();
    					  if (value<1000)
    						  split[split.length-1]=bg.longValue()+ " millisecs";
    					  else if (value < 60000)
    						  split[split.length-1]=value/1000F+ " secs";
    					  else if (value < 3600000)
    						  split[split.length-1]=value/60000F+ " mins";
    					  else if (value < 86400000F)
    						  split[split.length-1]=value/3600000F+ " hours";
    					  else
    						  split[split.length-1]=value/86400000F+ " days";
    				  }
    			  }
    			  catch(Exception err)
    			  {
  ...
  private String getNodeByReference(String m_label) {
	  for(Node n : m_aNodes)
	  {
		  if (n.getRefer().equals(m_label))
			  return n.getLabel();
	  }
	  assert(false);
	  return "";
