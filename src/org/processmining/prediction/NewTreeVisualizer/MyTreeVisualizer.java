...
public class MyTreeVisualizer extends PrintablePanel implements
 ...
 * @param classType 
   */
  public MyTreeVisualizer(TreeDisplayListener tdl, String dot, NodePlace p, Map<String, Color> map, 
		  Map<String, PredictionType> variableType, Character charDelimiter, float maxValue, PredictionType classType) 
  {
    super();
    this.map=map;
    initialize();
    ...
    TreeBuild builder = new TreeBuild(map,classType);

    Node n = null;
    // NodePlace arrange = new PlaceNode2(); NOT USED
    n = builder.create(new StringReader(dot),variableType,charDelimiter, maxValue); 
   ...
   * @param p the algorithm to be used to position the nodes.
   */
  public MyTreeVisualizer(TreeDisplayListener tdl, Node n, NodePlace p) {
    super();
  ...
  private void drawText(int x1, int y1, int s, boolean e_or_n, Graphics g) {
    ...
    if (m_FontColor == null) {
    	if (e_or_n)
    		g.setColor(Color.BLACK); //Color for edges is always black!
    	else
    		g.setColor(m_nodes[s].m_node.getFontColor());
    } else {
