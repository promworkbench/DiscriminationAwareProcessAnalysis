...
public static int saDepIndex=0,saFavIndex=0;
 ...
 public void setDiscParam(Instances data){
  ...
  public void relabeling(){
      ...
      leafRanking();
      int j = 0;
      while(leafRank[j][0] != 0) {
    	  System.out.println("leafRank["+j+"] =: \t"+leafRank[j][0]+"  "+leafRank[j][1]);
    	  j++;
    	  if (j > 100) {
    		  break;
    	  }
      ...
      System.out.println("init Discrimination =: \t"+getDisc(disc)); 
       while(getDisc(disc)>epsilon){
        	System.out.println("Discrimination =: \t"+getDisc(disc));
            changes++;
       ...
       System.out.println("after Discrimination =: \t"+getDisc(disc));
...
/*              for (int j=0; j<localModel().distribution().m_perClassPerBag.length; j++) {
                  temp=localModel().distribution().m_perClassPerBag[j][0];
                  localModel().distribution().m_perClassPerBag[j][0]=localModel().distribution().m_perClassPerBag[j][1];
                  localModel().distribution().m_perClassPerBag[j][1]=temp;
              }*/
           stop=true;
        }
            }
 ...
 //double [][] temp=new double[2][2];
 	...
 	public void infoDiscriminationTree() {
 	    ...
 	    if (m_isLeaf){
 	       ...
 	       f=localModel().distribution().m_sa[1];
 	       c=localModel().distribution().maxClass();
 	       System.out.println(" \n per class 0 :    "+ localModel().distribution().m_perClass[0] +"  per class 1 :"+localModel().distribution().m_perClass[1]);
 	       System.out.println(" protected :    "+s +"  favorable  :"+f);
 	       System.out.println(" class :    "+ c );
 	       totals = totals + s;
 	       totalf = totalf + f;
 	       totalClass0 = totalClass0 + localModel().distribution().m_perClass[0];
 	       totalClass1 = totalClass1 + localModel().distribution().m_perClass[1];
 	   } else {
 	         for (i=0;i<m_sons.length;i++){
 	            son(i).infoDiscriminationTree();
 	         }
 	    }
 	    
 	  System.out.println("\ntotal sensitive : "+ totals);
 	  System.out.println("total protected : "+ totalf);
 	  System.out.println("total class 0 : "+ totalClass0);
 	  System.out.println("total class 1 : "+ totalClass1);
 	}
