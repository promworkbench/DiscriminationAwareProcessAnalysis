...
public class Distribution implements Cloneable, Serializable {
  ...
  // Weight of Deprecated instances, m_sa[0], and Weight of Favorable instances, m_sa[1].
  public double m_sa[]=new double[2]; 
...
//m_sa=new double[2];
  ...
  public Distribution(Distribution toMerge) {
    ...
    m_sa=new double[saLength];
    System.arraycopy(toMerge.m_sa,0,m_sa,0,saLength);
...
//m_sa=new double[2];
  ...
  public final void add(int bagIndex,Instance instance) 
    ...
    int sa_index=Discrimination.getSaIndex();
       if(instance.toString(sa_index).equals(Discrimination.getSaDep()))
           m_sa[0]+=weight;
       else
           m_sa[1]+=weight;
  }
  ...
  public final void addRange(int bagIndex,Instances source,
    ...
    for (i = startIndex; i < lastPlusOne; i++) {
      ...
      int sa_index=Discrimination.getSaIndex();
       if(instance.toString(sa_index).equals(Discrimination.getSaDep()))
           m_sa[0]+=instance.weight();
       else
           m_sa[1]+=instance.weight();
    }
...
public final double[][] getDisc() {
       ...
       double [][] temp=new double[2][2];
        temp[0][maxClass()]=m_perBag[0];
         temp[1][maxClass()]=m_perBag[1];
         System.out.println("\n"+tot+"\tMale: "+m_perBag[1]+"\t Male&+ : "+m_perClassPerBag[1][1]+"\tFemale: "+m_perBag[0]+"\t Female&+ : "+m_perClassPerBag[0][1]);
      return temp;
    //return (m_perClassPerBag[1][1]/(double)m_perBag[1])-(m_perClassPerBag[0][1]/(double)m_perBag[0])*100;
  }
public final double getNumChanges() {
       double tot = totaL;
       int a =m_perBag.length;
       //System.out.println("\n"+a+"\tMale: "+m_perBag[1]+"\t Male&+ : "+m_perClassPerBag[1][1]+"\tFemale: "+m_perBag[0]+"\t Female&+ : "+m_perClassPerBag[0][1]);
    return ((((m_perBag[0]+0.001)*(m_perClassPerBag[1][1]+0.001))-((m_perBag[1]+0.001)*(m_perClassPerBag[0][1]+0.001)))/(double)(m_perBag[0]+0.001+m_perBag[1]))*2;
