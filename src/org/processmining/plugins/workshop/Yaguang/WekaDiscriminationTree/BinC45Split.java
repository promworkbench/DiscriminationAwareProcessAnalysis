...
public class BinC45Split extends ClassifierSplitModel{
  ...
  private void handleEnumeratedAttribute(Instances trainInstances)
	...
	if (secondDistribution.check(m_minNoObj)){
	  ...
	  if ((i == 0) || Utils.gr(currGR,m_gainRatio)){
	      //System.out.println(m_gainRatio+"\t"+currGR);
              m_gainRatio = currGR;
  ...
  // computeDiscLoss(trainInstances); 
   ...
   m_infoGain=((m_infoGain)*alpha + (1-netInfoGain)*(1-alpha));
   m_gainRatio =((m_gainRatio)*alpha + (1-netGain)*(1-alpha));
 ...
 c = trainInstances.classIndex();
    //trainInstances.setClassIndex(weka.classifiers.discrimination.Discrimination.getSaIndex()); 
   //  trainInstances.setClassIndex(6); 
   Distribution m_dist,nDistribution,sDistribution;
    int numAttValues;
    double currIG,currGR;
    Instance instance;
    int i;
    ...
    numAttValues = trainInstances.attribute(m_attIndex).numValues();
    nDistribution = new Distribution(numAttValues,
				       trainInstances.numClasses());
    ...
    while (enu.hasMoreElements()) {
      instance = (Instance) enu.nextElement();
      if (!instance.isMissing(m_attIndex))
	nDistribution.add((int)instance.value(m_attIndex),instance);
    }
   // m_distribution= nDistribution;

    // For all values
    for (i = 0; i < numAttValues; i++){

      if (Utils.grOrEq(nDistribution.perBag(i),m_minNoObj)){
	sDistribution = new Distribution(nDistribution,i);
	
	// Check if minimum number of Instances in the two
	// subsets.
	if (sDistribution.check(m_minNoObj))
        {
	  m_numSubsets = 2;
	 currIG = m_infoGainCrit.splitCritValue(sDistribution,
					       m_sumOfWeights);
	 currGR = m_gainRatioCrit.splitCritValue(sDistribution,
						m_sumOfWeights,
						currIG);
	  if ((i == 0) || Utils.gr(currGR,m_gainRatio)){
	   ...
	   m_gainRatio = currGR;
	  m_infoGain = currIG;
          //System.out.println("Att index + "+m_attIndex+"\t"+m_gainRatio+"\t"+m_infoGain);
	 // m_splitPoint = (double)i;
	  // m_distribution = sDistribution;
	  }
	}
      }
    }
